...
def random_mixed_kernels(
    ...
    assert rand_num >= 0 and rand_num <= 1, 'rand_num should be in (0, 1).'
...
def add_jpg_compression(
    img: np.ndarray,
    quality: float | int = 90,
) -> np.ndarray:
    """Add JPG compression artifacts.
    Args:
        img (Numpy array): Input image, shape (h, w, c), range [0, 1], float32.
        quality (float | int): JPG compression quality. 0 for lowest quality, 100 for
...
def random_add_jpg_compression(
    img: np.ndarray,
    quality_range: tuple[float | int, float | int] = (90, 100),
) -> np.ndarray:
